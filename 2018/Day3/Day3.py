...
def intersection(A,B,C,D):
    ...
    if nA.x == nB.x:
        ...
        if ((nC.x > nA.x and nC.x < nB.x) and (nA.y > nC.y and nA.y < nD.y)):
...
for i in range(0,len(wireA_instructions)):
    ...
    wireAlenght.append(wireAlenght[i]+ abs(int(wireA_instructions[i][1::])))

wireB = [Point(0,0)]
for i in range(0,len(wireB_instructions)):
    wireB.append(makeMove(wireB[i],wireB_instructions[i]))
    wireBlenght.append(wireBlenght[i]+ abs(int(wireB_instructions[i][1::])))
    ...
    for j in range(1,len(wireA)):
       if not(checkParallel(wireB[i-1],wireB[i],wireA[j-1],wireA[j])):
           ...
           if tempPoint != None:
               ...
               if wireBlenght[i] + wireAlenght[j] < minPath:
                   minPath = wireBlenght[i-1] + wireAlenght[j-1] + int(
                 math.sqrt(pow((wireB[i-1].x-tempPoint.x),2) + pow((wireB[i-1].y-tempPoint.y),2))  
                + math.sqrt(pow((wireA[j-1].x-tempPoint.x),2)+pow((wireA[j-1]).y-tempPoint.y,2)))
...
print ("min distance", distance ,"but min path is", minPath)      
